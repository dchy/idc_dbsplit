        ...
        if (flag == 1) {
            ...
            $(this).mouseleave(function () {
                if (now_cli >= 0) {
                    ...
                    }//2017/3/21 新加客服后台-售后-工程经理详情页.html-售后单维修难度评分
                    if(atti){
                    	atti.text(options.types[parseInt(now_cli)]);
                    }
                } else {
                    startParent.find("a").removeClass("clibg");
                    if (countScore) {
    ...
    e.fn.extend({
        slimScroll: function (n) {
            var i = e.extend({
                ...
                start: "top",
                opacity: 0,
                //opacity: .4,////hujianghua改过此处,解决的问题是（页面一加载进来不想让滚动条默认显示出来，想实现的效果是鼠标移入是再显示滚动条(如下：)
                alwaysVisible: !1,
                disableFadeOut: !1,
                railVisible: !1,
                railColor: "#333",
            ...
            return this.each(function () {
                ...
                function s() {
                    ...
                    g = f,
                    //hujianghua改过此处,解决的问题是（页面一加载进来不想让滚动条默认显示出来，想实现的效果是鼠标移入是再显示滚动条
                    m >= y.outerHeight() ? v = !0 : (_.stop(!0, !0).css({ opacity: "0.4" }).fadeIn("fast"),
                    i.railVisible && x.stop(!0, !0).css({ opacity: "0.4" }).fadeIn("fast"))
                }
                function l() {
                ...
                if (y.parent().hasClass(i.wrapperClass)) {
                    ...
                    var x = e("<div></div>").addClass(i.railClass).css({
                      ...
                      , _ = e("<div></div>").addClass(i.barClass).css({//slimScrollBar
                          background: i.color,
                          width: i.size,
                          position: "absolute",
                          top: 0,
